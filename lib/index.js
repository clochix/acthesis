...
(function () {
  ...
  module.exports = function (options) {
    ...
    server.on('connection', function (socket) {
      ...
      eventEmitter.on('push', sendPush);
      socket.on('disconnect', function () {
        eventEmitter.removeListener('activityWaiting', sendPending);
        eventEmitter.removeListener('push', sendPush);
      });
    });

    return function (req, res, next) {
      //jshint maxcomplexity: 20, maxstatements: 50
      var registered, timeout, pending, domRequest, path, requester, alarmId, id, version, registration, base, port;
      ...
      switch (path.slice(0, 3).join('/')) {
        ...
        case '/activity/unregister':
          ...
          break;
        case '/activity/debug':
          res.send(JSON.stringify(activity.debug()));
          break;
        case '/alarm':
          if (req.method === 'GET') {
            ...
            req.body.type    = 'alarm';
            alarmId = activity.alarm.add(requester, req.body);
            req.body.id = alarmId;

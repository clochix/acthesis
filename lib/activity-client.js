...
/*exported Acthesis, ActivityHandlerDescription */
...
function Acthesis(options, manifest) {
  ...
  if (Object.keys(registered).length > 0) {
    ...
    socket.on('open', function(){
      socket.send(JSON.stringify({type: 'providerUrl', data: {name: document.title, url: selfUrl}}));
